...
export function SystemSettingsPage({ onNavigateToIntegrations }: SystemSettingsPageProps) {
  ...
  const { data, loading, error, refetch } = useSystemSettings(selectedCategory === 'all' ? undefined : selectedCategory);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (editingSetting) {
        await SupabaseDatabaseService.updateSystemSetting(editingSetting.id, {
        ...
        await SupabaseDatabaseService.createSystemSetting({
          key: formData.key,
          value: formData.value,
          description: formData.description,
          category: formData.category,
  ...
  const handleDelete = async (settingId: string) => {
    if (window.confirm('Are you sure you want to delete this setting?')) {
      try {
        await SupabaseDatabaseService.deleteSystemSetting(settingId);
        refetch();
      } catch (error) {
